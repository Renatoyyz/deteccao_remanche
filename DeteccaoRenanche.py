...
class PredictFromCamera(QWidget):
    ...
    def init_ui(self):
        ...
        self.predict_btn = QPushButton("Predizer Áreas na Imagem/Fotografia")
        self.predict_btn.clicked.connect(self.predict_on_image)
        ...
        self.output_text.setStyleSheet("border: 1px solid #aaa; background: #f8f8f8; min-height: 80px; padding: 6px;")
    ...
    def predict_on_image(self):
        if not self.areas or not self.model_path:
            ...
            return
        self.load_neural_ai()
        # Decide qual imagem usar: capturada ou carregada
        img = None
        if self.captured_img is not None:
            img = self.captured_img.copy()
            # Atualiza visualização se vier da câmera
            if self.image_path is None and img is not None:
            ...
            # Se não há imagem carregada, tenta capturar da câmera
            ...
            while True:
                ...
                if key == ord('s'):
                    img = frame.copy()
                    # Atualiza visualização da foto capturada
                    height, width, channel = img.shape
                    bytes_per_line = 3 * width
                    qimg = QImage(img.data, width, height, bytes_per_line, QImage.Format_RGB888).rgbSwapped()
                    pixmap = QPixmap.fromImage(qimg)
        ...
        for area in self.areas:
            ...
            # Predição para classe_nao_passa
    ...
    def select_model(self):
        path, _ = QFileDialog.getOpenFileName(self, "Selecionar Modelo", "", "Modelos HDF5 (*.hdf5)")
        if path:
            self.model_path = path
            self.status_label.setText(f"Modelo selecionado: {os.path.basename(path)}")
    ...
    def load_neural_ai(self):
        ...
        if self.class_txt_path:
            self.neural_ai.img_list = np.loadtxt(self.class_txt_path, dtype=str)
            self.neural_ai.img_list = self.neural_ai.img_list.tolist()
